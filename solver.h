...
public:
  ...
  struct decision_point {
    decision_point(prolog_timestamp timestamp, std::vector<goal> &goal_stack,
                   int next_choice_number):
      timestamp(timestamp), goal_stack(std::move(goal_stack)),
      next_choice_number(next_choice_number) {
    }
...
private:
  ...
  bool apply_clause(clause_index clause_idx, int choice_number,
                    bool add_decision_point = true);
